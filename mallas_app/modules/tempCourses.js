...
export function ensureDraftTempCourses(draft) {
  ...
  // Keep suppressed_courses aligned with temp overrides (orig_course_id).
  const overrideIds = new Set();
  for (const c of draft.temp_courses) {
    if (!c || typeof c !== "object") continue;
    if (!c.orig_course_id) continue;
    const origId = String(c.orig_course_id);
    overrideIds.add(origId);
    if (!draft.suppressed_courses.includes(origId)) draft.suppressed_courses.push(origId);
  }
