...
export function computeWarnings(terms, courses, placements, draft, config) {
  ...
  // Rule:
  // - OK if the co-requisite is approved, or scheduled in the same term or any earlier term.
  ...
  for (const c of Array.isArray(courses) ? courses : []) {
    ...
    for (const q of coreqs) {
      ...
      // Must be in the same term or any previous term.
      if (tid != null && Number.isFinite(qIdx) && qIdx <= cIdx) continue;
      ...
      pushWarn("hard", {
        ...
        term_id: tid,
        coreq: q,
        text: `${sigla || "Curso"} requiere correquisito ${q} en el mismo semestre o en un semestre anterior.`,
      });
    }
  }
